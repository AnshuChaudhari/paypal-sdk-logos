...
export const ACH_LOGO_COLORS: LogoColorMap = {
  [LOGO_COLOR.DEFAULT]: {
    primary: "#0EE06E",
  },
  [LOGO_COLOR.WHITE]: {
...
export const getAchSVG = ({ primary }: LogoColors): ElementNode => {
  return (
    <svg
      width="95"
      height="20"
...
export function AchLogoExternalImage({
  logoColor = LOGO_COLOR.BLACK,
  ...props
}: {
  logoColor?: $Values<typeof LOGO_COLOR>,
}): ComponentNode<SVGLogoProps> {
  const cdnUrl = getLogoCDNUrl(LOGO.TRUSTLY, ACH_LOGO_COLORS, logoColor);

  return <SVGLogo {...props} name={LOGO.TRUSTLY} cdnUrl={cdnUrl} />;
}

export function AchLogoInlineSVG({
  logoColor = LOGO_COLOR.BLACK,
  ...props
}: {
  logoColor?: $Values<typeof LOGO_COLOR>,
  ...
  const svg = getAchSVG(
    getLogoColors(LOGO.TRUSTLY, ACH_LOGO_COLORS, logoColor)
  );
...
export const AchLogo = AchLogoInlineSVG;
